...
class PacmanSearchTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
            ...
            gold_expanded = max(\
                int(solutionDict['expanded_nodes']),\
                     int(solutionDict['rev_expanded_nodes']), \
                         int(solutionDict['alt_expanded_nodes']), \
                             int(solutionDict['alt_rev_expanded_nodes']))
        ...
        if solution not in gold_solution:
            ...
            if 'alt_solution' in solutionDict and 'alt_rev_solution' in solutionDict:
                grades.addMessage('\tcorrect alternative solution length:\n%s' % len(gold_solution[2]))
                grades.addMessage('\tcorrect alternative solution:\n%s' % wrap_solution(gold_solution[2]))
                grades.addMessage('\tcorrect alternative (reversed) solution length:\n%s' % len(gold_solution[2]))
                grades.addMessage('\tcorrect alternative (reversed) solution:\n%s' % wrap_solution(gold_solution[2]))
            return False
